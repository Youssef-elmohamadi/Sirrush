...
const BlogHero = () => {
  const t = useTranslations("blog_page.hero");
    ...
    <section className="relative h-[400px] flex items-center justify-center overflow-hidden">
      ...
      <div className="container-lg relative z-10 text-center text-white">
        ...
        <div className="flex flex-col items-center gap-6">
          <p className="text-pure-white/70 max-w-3xl text-lg font-medium leading-relaxed italic animate-fade-in-up">
            {t("subtitle")}
          </p>
          
          <nav className="flex items-center gap-4 text-sm font-bold uppercase tracking-[0.3em] mt-6 bg-white/10 px-8 py-3 rounded-full backdrop-blur-xl border border-white/20">
            <Link href="/" className="hover:text-ant-gold transition-colors">
              {t("breadcrumb_home")}
            </Link>
            <span className="text-ant-gold">/</span>
            <span className="text-pure-white/40">{t("breadcrumb_blog")}</span>
          </nav>
        </div>
      </div>
    </section>
